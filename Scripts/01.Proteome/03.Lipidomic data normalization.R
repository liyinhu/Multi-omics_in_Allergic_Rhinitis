# Usage:
#   Rscript "03.Lipidomic data normalization.R"
#   Rscript "03.Lipidomic data normalization.R" --lipid lipid_matrix.txt --info sample_info.txt --outdir results
...
lipid_path <- parse_cli_value(args, c("--lipid", "-l"))
...
if (is.null(lipid_path) || !nzchar(lipid_path)) {
  lipid_path <- file.choose()
}
if (is.null(info_path) || !nzchar(info_path)) {
  info_path <- file.choose()
}

data <- read_delim_checked(lipid_path, "Lipid matrix")
info <- read_delim_checked(info_path, "Sample info")

if (nrow(data) == 0 || ncol(data) == 0) {
  stop("Lipid matrix must contain samples in rows and lipids in columns.")
}
if (nrow(info) == 0 || ncol(info) == 0) {
  stop("Sample info table is empty.")
}
if (is.null(rownames(data)) || is.null(rownames(info))) {
  stop("Lipid matrix and sample info must both have sample IDs in the first column / row names.")
}
...
if (length(common_samples) == 0) {
  stop("Lipid matrix and sample info have no overlapping sample IDs.")
}
...
if (!identical(rownames(data), rownames(info))) {
  stop("Lipid matrix and sample info could not be aligned to the same sample order.")
}

lipid_numeric <- suppressWarnings(
  as.data.frame(lapply(data, function(x) as.numeric(as.character(x))), check.names = FALSE)
)
rownames(lipid_numeric) <- rownames(data)

bad_lipid_cols <- names(data)[vapply(
  seq_along(data),
  function(i) any(is.na(lipid_numeric[[i]]) & !is.na(data[[i]])),
  logical(1)
)]
if (length(bad_lipid_cols) > 0) {
  stop(sprintf(
    "Lipid matrix must contain only numeric lipid expression values. Invalid columns: %s",
    paste(bad_lipid_cols, collapse = ", ")
  ))
}
...
lipid_count <- ncol(data)
if (lipid_count != 362) {
  warning(sprintf(
    "Expected 362 lipid columns based on the original script, but detected %d. Continuing with the detected lipid columns.",
    lipid_count
  ))
}
...
Adjusted_data <- data.frame(matrix(ncol = lipid_count, nrow = nrow(data)))
colnames(Adjusted_data) <- colnames(data)
rownames(Adjusted_data) <- rownames(data)

Age_Sex_Effects_on_lipids <- data.frame(matrix(ncol = 8, nrow = lipid_count))
colnames(Age_Sex_Effects_on_lipids) <- c(
  "Age_Estimate", "Age_SE", "Age_T_value", "Age_P_value",
  "Sex_Estimate", "Sex_SE", "Sex_T_value", "Sex_P_value"
)
rownames(Age_Sex_Effects_on_lipids) <- colnames(data)

# Data normalization: linear regression to correct the effects of age and sex
Merge_data <- cbind(data, info)

for (k in seq_len(lipid_count)) {
  lipid_name <- colnames(data)[k]
  formula_text <- sprintf("`%s` ~ Age + Gender", lipid_name)
  ...
  if (length(age_row) == 1) {
    Age_Sex_Effects_on_lipids[k, 1:4] <- as.vector(coef_summary[age_row, 1:4])
  }
  if (length(gender_rows) >= 1) {
    Age_Sex_Effects_on_lipids[k, 5:8] <- as.vector(coef_summary[gender_rows[1], 1:4])
    if (length(gender_rows) > 1) {
      warning(sprintf(
        "Lipid %s has multiple Gender coefficients; only the first Gender coefficient is written to AgeSex_Effects.csv.",
        lipid_name
      ))
    }
  }
  ...
  Adjusted_data[, k] <- Merge_data[[lipid_name]] - as.numeric(adjustment)
}

# Export correction coefficients and adjusted results
write.csv(Adjusted_data, file.path(outdir, "Agesex_adjusted_lipidites.csv"), row.names = TRUE)
write.csv(Age_Sex_Effects_on_lipids, file.path(outdir, "AgeSex_Effects.csv"), row.names = TRUE)
