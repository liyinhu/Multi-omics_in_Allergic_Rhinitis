# Usage:
#   Rscript "02.Differential metabolite analysis.R"
#   Rscript "02.Differential metabolite analysis.R" --input adjusted_metabolome.csv --outdir results --group-column Group
...
# Preserve the original compatibility assumption: the first two columns are non-metabolite columns.
metabolite_col_idx <- seq.int(3, ncol(df))
if (length(metabolite_col_idx) == 0 || max(metabolite_col_idx) > ncol(df)) {
  stop("At least one metabolite feature column is required from the third column onward.")
}

metabolite_df <- df[, metabolite_col_idx, drop = FALSE]
if (ncol(metabolite_df) == 0) {
  stop("No metabolite feature columns were found from the third column onward.")
}

metabolite_df_numeric <- suppressWarnings(
  as.data.frame(lapply(metabolite_df, function(x) as.numeric(as.character(x))), check.names = FALSE)
)

bad_cols <- names(metabolite_df)[vapply(
  seq_along(metabolite_df),
  function(i) any(is.na(metabolite_df_numeric[[i]]) & !is.na(metabolite_df[[i]])),
  logical(1)
)]

if (length(bad_cols) > 0) {
  stop(sprintf(
    "Metabolite feature columns must be numeric or convertible to numeric. Invalid columns: %s",
    paste(bad_cols, collapse = ", ")
  ))
}

df[, metabolite_col_idx] <- metabolite_df_numeric

Number_of_metabolites <- ncol(metabolite_df_numeric)
...
write.csv(Test_summary, file.path(outdir, "Metabolome_summary.csv"), row.names = TRUE)
...
volcano_plot <- ggplot(
  ...
  labs(
    ...
    y = "-log10(P-value)",
    title = "Plasma metabolites (AR vs HC)"
  ) +
  theme(
...
ggsave(
  filename = file.path(outdir, "Metabolome_volcano.pdf"),
  plot = volcano_plot,
  width = 8,
  height = 6
)
