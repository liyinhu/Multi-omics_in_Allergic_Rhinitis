# Usage:
#   Rscript "01.Metabolic data normalization.R"
#   Rscript "01.Metabolic data normalization.R" --metabolite metabolite_matrix.txt --info sample_info.txt --outdir results
...
metabolite_path <- parse_cli_value(args, c("--metabolite", "-m"))
...
if (is.null(metabolite_path) || !nzchar(metabolite_path)) {
  metabolite_path <- file.choose()
}
if (is.null(info_path) || !nzchar(info_path)) {
  info_path <- file.choose()
}

data <- read_delim_checked(metabolite_path, "Metabolite matrix")
info <- read_delim_checked(info_path, "Sample info")

if (nrow(data) == 0 || ncol(data) == 0) {
  stop("Metabolite matrix must contain samples in rows and metabolites in columns.")
}
if (nrow(info) == 0 || ncol(info) == 0) {
  stop("Sample info table is empty.")
}
if (is.null(rownames(data)) || is.null(rownames(info))) {
  stop("Metabolite matrix and sample info must both have sample IDs in the first column / row names.")
}
...
if (length(common_samples) == 0) {
  stop("Metabolite matrix and sample info have no overlapping sample IDs.")
}
...
if (!identical(rownames(data), rownames(info))) {
  stop("Metabolite matrix and sample info could not be aligned to the same sample order.")
}

metabolite_numeric <- suppressWarnings(
  as.data.frame(lapply(data, function(x) as.numeric(as.character(x))), check.names = FALSE)
)
rownames(metabolite_numeric) <- rownames(data)

bad_metabolite_cols <- names(data)[vapply(
  seq_along(data),
  function(i) any(is.na(metabolite_numeric[[i]]) & !is.na(data[[i]])),
  logical(1)
)]
if (length(bad_metabolite_cols) > 0) {
  stop(sprintf(
    "Metabolite matrix must contain only numeric metabolite expression values. Invalid columns: %s",
    paste(bad_metabolite_cols, collapse = ", ")
  ))
}

data <- metabolite_numeric
...
metabolite_count <- ncol(data)
if (metabolite_count != 362) {
  warning(sprintf(
    "Expected 362 metabolite columns based on the original script, but detected %d. Continuing with the detected metabolite columns.",
    metabolite_count
  ))
}
...
Adjusted_data <- data.frame(matrix(ncol = metabolite_count, nrow = nrow(data)))
colnames(Adjusted_data) <- colnames(data)
rownames(Adjusted_data) <- rownames(data)

Age_Sex_Effects_on_metabolites <- data.frame(matrix(ncol = 8, nrow = metabolite_count))
colnames(Age_Sex_Effects_on_metabolites) <- c(
  "Age_Estimate", "Age_SE", "Age_T_value", "Age_P_value",
  "Sex_Estimate", "Sex_SE", "Sex_T_value", "Sex_P_value"
)
rownames(Age_Sex_Effects_on_metabolites) <- colnames(data)

# Data normalization: linear regression to correct the effects of age and sex
Merge_data <- cbind(data, info)

for (k in seq_len(metabolite_count)) {
  metabolite_name <- colnames(data)[k]
  formula_text <- sprintf("`%s` ~ Age + Gender", metabolite_name)
  ...
  if (length(age_row) == 1) {
    Age_Sex_Effects_on_metabolites[k, 1:4] <- as.vector(coef_summary[age_row, 1:4])
  }
  if (length(gender_rows) >= 1) {
    Age_Sex_Effects_on_metabolites[k, 5:8] <- as.vector(coef_summary[gender_rows[1], 1:4])
    if (length(gender_rows) > 1) {
      warning(sprintf(
        "Metabolite %s has multiple Gender coefficients; only the first Gender coefficient is written to AgeSex_Effects.csv.",
        metabolite_name
      ))
    }
  }
  ...
  Adjusted_data[, k] <- Merge_data[[metabolite_name]] - as.numeric(adjustment)
}

# Export correction coefficients and adjusted results
write.csv(Adjusted_data, file.path(outdir, "Agesex_adjusted_metabolites.csv"), row.names = TRUE)
write.csv(Age_Sex_Effects_on_metabolites, file.path(outdir, "AgeSex_Effects.csv"), row.names = TRUE)
