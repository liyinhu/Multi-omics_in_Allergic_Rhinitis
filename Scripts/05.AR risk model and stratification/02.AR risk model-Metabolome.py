"""
Usage:
  python 02.AR risk model-Metabolome.py
  python 02.AR risk model-Metabolome.py -i Target_metabolome.csv -o .
  python 02.AR risk model-Metabolome.py --input Target_metabolome.csv --outdir results --label-column Group
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a random forest metabolome AR classifier and export ROC/SHAP outputs.")
    parser.add_argument("-i", "--input", default="Target_metabolome.csv", help="Input CSV file (default: Target_metabolome.csv)")
...
plt.tight_layout()
plt.savefig(outdir / "ROC_curve_metabolome.pdf", format="pdf", bbox_inches="tight")
plt.show()
plt.close(fig)
...
feature_importance.to_csv(outdir / "Shap_feature_importance_metabolome.csv", index=False)
