"""
Usage:
  python 03.AR risk model-Lipidome.py
  python 03.AR risk model-Lipidome.py -i Target_Lipidome.csv -o .
  python 03.AR risk model-Lipidome.py --input Target_Lipidome.csv --outdir results --label-column Group
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a random forest lipidome AR classifier and export ROC/SHAP outputs.")
    parser.add_argument("-i", "--input", default="Target_Lipidome.csv", help="Input CSV file (default: Target_Lipidome.csv)")
...
plt.tight_layout()
plt.savefig(outdir / "ROC_curve_lipidome.pdf", format="pdf", bbox_inches="tight")
plt.show()
plt.close(fig)
...
feature_importance.to_csv(outdir / "Shap_feature_importance_lipidome.csv", index=False)
