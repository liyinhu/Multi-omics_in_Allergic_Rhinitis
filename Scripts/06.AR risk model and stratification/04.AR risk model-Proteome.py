"""
Usage:
  python 04.AR risk model-Proteome.py
  python 04.AR risk model-Proteome.py -i Target_Proteome.csv -o .
  python 04.AR risk model-Proteome.py --input Target_Proteome.csv --outdir results --label-column Group
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a random forest proteome AR classifier and export ROC/SHAP outputs.")
    parser.add_argument("-i", "--input", default="Target_Proteome.csv", help="Input CSV file (default: Target_Proteome.csv)")
...
plt.tight_layout()
plt.savefig(outdir / "ROC_curve_proteome.pdf", format="pdf", bbox_inches="tight")
plt.show()
plt.close(fig)
...
feature_importance.to_csv(outdir / "Shap_feature_importance_proteome.csv", index=False)
